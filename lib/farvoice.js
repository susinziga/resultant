...
const getAccessToken = async ({ tokenUrl, clientId, clientSecret, scope }) => {
  ...
  const now = Date.now();
  if (tokenCache.accessToken && now < tokenCache.expiresAt - REFRESH_BEFORE_MS) {
  ...
  if (!res.ok) {
    const err = new Error(
      `Farvoice token request failed: ${res.status} ${res.statusText}`
    );
...
export const sendFarvoiceEmail = async ({
  ...
  const res = await fetch(`${baseUrl}/api/v2/ingest/email`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    const err = new Error(
      `Farvoice request failed: ${res.status} ${res.statusText}`
    );
    err.status = res.status;
    err.response = data;
    throw err;
